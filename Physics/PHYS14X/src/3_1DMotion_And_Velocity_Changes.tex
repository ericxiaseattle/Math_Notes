...
%
%    \textbf{Kinematics}: only describes motion, it does not explain changes in motion.
%
%    \begin{center}
%        \begin{tabular}{|c|c|}
%            \hline
...
%            \textbf{Time}       & $t$ \\
%            \hline
%            \textbf{Displacement}   & $\Delta x \equiv x_2 - x_1$ \\
%            \hline
%            \textbf{Time interval}  & $\Delta t \equiv t_2 - t_1$ \\
%            \hline
%            \textbf{Distance}       & $s$ \\
%            \hline
%            \textbf{Average Velocity}   & $v_{avg} \equiv \frac{\Delta x}{\Delta t}  = \frac{x_2 - x_1}{t_2 - t_1}$ \\
%            \hline
%            \textbf{Position after Change}  & $x_2 = x_1 + v_{avg} \Delta t$ \\
%            \hline
%        \end{tabular}
%    \end{center}
%
%    \noindent We can plot an object's position at an instant $t$ using a number line and a clock. It is important to note that distance ($s$) is not the same as displacement ($\Delta x$). If all travel is in the
%    same direction, then $s=|\Delta x|$. \\
%
%    \noindent $\Delta \overrightarrow{x}$ and $\overrightarrow{v_{avg}}$ are vectors, having both magnitude and direction. \\
%
%    \noindent \textbf{Component Vector Notation:}
%
%    \[
%        \overrightarrow{v} = v \bm{\hat{i}}
%    \]
%
%    \noindent where $\bm{\hat{i}}$ is a unit vector in the positive $x$-direction, $\bm{\hat{j}}$ is a unit vector in the positive $y$-direction, and $\bm{\hat{k}}$ is a unit vector in the positive $z$-direction. \\
%
%    \noindent Below is a graph of an object with constant velocity. The blue line describes a negative constant velocity and the object is moving away from the origin in the negative direction. The red line describes a
%    positive constant velocity and the object is moving away from the origin in the positive direction. We know that the lines describe constant velocities because their slopes on the position-time graph are constant
%    throughout.
%
%    \begin{center}
...
%    \end{center}
%
%    \noindent \textbf{Instantaneous velocity} is when the ratio $\frac{\Delta x}{\Delta t}$ is changing in time. We can always still find a short enough time interval over which the ratio seems to be constant. Thus, if
%    we represent $\Delta t \rightarrow dt$ and $\Delta x \rightarrow dx$, then $v_{avg} \rightarrow \frac{dx}{dt}$, then in this case the ratio $\frac{dx}{dt}$ describes instantaneous velocity, where
%
%    \[
%        v \equiv \lim _{\Delta t \rightarrow 0} \frac{\Delta x}{\Delta t} = \frac{dx}{dt}
%    \]
%
%    \noindent If the motion of two objects were plotted on the same position-time graph, then their instantaneous velocities would be the same at any instant both motion curves had the same slope of the tangent line to
%    the curves at said instant.
